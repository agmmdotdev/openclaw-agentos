...
test('lazy artifact preserves configuration validation, locale and complete highlighting APIs', { timeout: 60000 }, async t => {
  ...
  const results = [];
  for (const [mode, core] of [['eager', eager], ['lazy', bundled], ['split', split.source]]) {
    ...
    await writeFile(entry, `let __testHighlightLoads=0,__testSchemaLoads=0;const __testMode=${JSON.stringify(mode === 'split' ? 'lazy' : mode)},__testSplit=${mode === 'split'};\n` + instrumented + '\n' + fixture, { flag: 'wx' });
