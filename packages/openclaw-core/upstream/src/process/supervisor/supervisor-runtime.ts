...
export function createProcessSupervisorWithAdapter(
  createAdapter: (input: SpawnInput) => Promise<SpawnProcessAdapter>,
  warnSpawnFailure?: (message: string) => Promise<void>,
): ProcessSupervisor & {
  ...
  const startRun = async (
    ...
    let stdoutRawListener = input.mode === "child" ? input.onStdoutRaw : undefined;
    let stderrRawListener = input.mode === "child" ? input.onStderrRaw : undefined;
    ...
    try {
      ...
      const adapter = await createAdapter(input);
      ...
      const deliverOutput = <Chunk>(listener: ((chunk: Chunk) => void) | undefined, chunk: Chunk) => {
        if (outputFailure) return;
        try { listener?.(chunk); }
        catch (error) {
          outputFailure = { error };
          requestCancel("spawn-error");
        }
      };
      const onRawOutput = (stream: "stdout" | "stderr") => (chunk: Buffer) => {
        deliverOutput(stream === "stdout" ? stdoutRawListener : stderrRawListener, chunk);
        touchOutput();
      };
      adapter.onStdout((chunk) => {
        ...
        deliverOutput(stdoutListener, chunk);
        touchOutput();
      }, stdoutRawListener ? onRawOutput("stdout") : undefined);
      adapter.onStderr((chunk) => {
        if (captureOutput) {
          stderr = appendCapturedOutput(stderr, chunk, "stderr", maxCapturedOutputChars);
        }
        deliverOutput(stderrListener, chunk);
        touchOutput();
      }, stderrRawListener ? onRawOutput("stderr") : undefined);
      ...
      const managedRun: ManagedRun = {
        ...
        detachOutput: () => {
          ...
          stdoutRawListener = undefined;
          stderrRawListener = undefined;
        },
      };
      ...
      await warnSpawnFailure?.(`spawn failed: runId=${runId} reason=${String(err)}`);
      throw err;
    }
  };
  ...
  const spawn = (input: SpawnInput): Promise<ManagedRun> => {
    ...
    if (active.has(runId) || startingRuns.has(runId)) {
      return Promise.reject(new Error(`process run already active: ${runId}`));
    }
