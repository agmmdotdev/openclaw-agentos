...
export type RunWorkerEmbeddedTurnParams = {
  toolRuntime?: { sandbox: NonNullable<Parameters<typeof createCoreCodingTools>[0]["sandbox"]> };
  agentId: string;
...
export async function runWorkerEmbeddedTurn(params: RunWorkerEmbeddedTurnParams): Promise<void> {
  ...
  const coreTools = createCoreCodingTools({
    ...(params.toolRuntime ? { sandbox: params.toolRuntime.sandbox } : {}),
    ...
    execDefaults: {
      ...
      host: params.toolRuntime ? "sandbox" : "gateway",
