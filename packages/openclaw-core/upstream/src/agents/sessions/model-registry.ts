...
// In-memory worker registries never parse catalogs; construct this private graph
// only when the catalog owner needs its validator.
function createModelsConfigSchema() {
...
type ModelsConfig = Static<ReturnType<typeof createModelsConfigSchema>>;
type ProviderAuthMode = NonNullable<ModelsConfig["providers"][string]["auth"]>;
let modelsConfigValidator:
  | ReturnType<typeof Compile<ReturnType<typeof createModelsConfigSchema>>>
  | undefined;
...
export class ModelRegistry {
  ...
  private loadCustomModels(
    ...
    try {
      ...
      const validateModelsConfig = (modelsConfigValidator ??= Compile(createModelsConfigSchema()));
