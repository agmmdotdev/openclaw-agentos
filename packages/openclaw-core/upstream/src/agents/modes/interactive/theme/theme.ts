...
// Headless tool use shares styling helpers without needing theme-file validation.
function createThemeJsonSchema() {
  ...
  return ThemeJsonSchema;
}

type ThemeJson = Static<ReturnType<typeof createThemeJsonSchema>>;
type ColorValue = ThemeJson["colors"]["accent"];
let themeJsonValidator:
  | ReturnType<typeof Compile<ReturnType<typeof createThemeJsonSchema>>>
  | undefined;
...
function parseThemeJson(label: string, json: unknown): ThemeJson {
  const validateThemeJson = (themeJsonValidator ??= Compile(createThemeJsonSchema()));
