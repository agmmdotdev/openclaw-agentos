...
function createPluginInstallRecordSchema() {
  ...
  return z.object(PluginInstallRecordShape).passthrough();
}

let pluginInstallRecordSchema: ReturnType<typeof createPluginInstallRecordSchema> | undefined;

/** Construct install validators only when a persisted record needs parsing. */
export function getPluginInstallRecordSchema() {
  return (pluginInstallRecordSchema ??= createPluginInstallRecordSchema());
}
