...
parser.add_argument('--allow-legacy', action='store_true', help='Read historical reports without frozen dependency provenance')
...
for trial in args.trials:
    ...
    for backend in reports:
        ...
        if not args.allow_legacy and snapshot is None: raise ValueError(f'Unfrozen dependencies: {path}')
        identity = snapshot['manifestSha256'] if snapshot else 'legacy'
        if expected_snapshot is None: expected_snapshot = identity
        if identity != expected_snapshot: raise ValueError(f'Mixed dependency snapshots: {path}')
        entry_hash = report['entrySha256']
        if backend in variant_hashes and variant_hashes[backend] != entry_hash: raise ValueError(f'Mixed core entries: {backend}')
        variant_hashes[backend] = entry_hash
    ...
    for key in ['environment', 'turns', 'sampleIntervalMs', 'benchmarkManifest', 'requestLauncherSha256', 'samplerSha256', 'harnessSha256', 'dependencySnapshot']:
        if pair[0].get(key) != pair[1].get(key): raise ValueError(f'Unmatched {key}: {trial}')

keys = ['firstProcessMs', 'subsequentCpuMedianSeconds', 'totalCpuSeconds', 'subsequentProcessMedianMs', 'peakPssMiB', 'warmTurnMedianMs', 'idleCorePssMiB']
summary = {}
for backend, rows in reports.items():
    ...
    if args.mode == 'request-cache':
        cold_cpu = [r['runs'][0]['cpuSeconds'] for r in rows]
        metrics['firstProcessCpuSeconds'] = {'median': statistics.median(cold_cpu), 'range': [min(cold_cpu), max(cold_cpu)]}
...
print(json.dumps({'trials': args.trials, 'mode': args.mode, 'sourceLayout': args.source_layout, 'snapshotManifestSha256': expected_snapshot, 'entryHashes': variant_hashes,
                  'method': 'Median of per-trial statistics; fresh paired controls; process-tree PSS; scripted inference and real tools.',
                  'summary': summary, 'sourceChangePercent': changes, 'inputs': inputs, 'summarizerSha256': hashlib.sha256(Path(__file__).read_bytes()).hexdigest()}, indent=2))
