...
entry='artifacts/core/native-sdk-supervised-benchmark.mjs' if args.backend=='sdk-upstream' else 'artifacts/core/native-sdk-core-benchmark.mjs' if args.backend=='sdk' else 'artifacts/core/native-core-benchmark.mjs'
...
def run(env,index):
    ...
    start=time.monotonic();events=[];samples=[];logs=[];buffers={};seen={}
    child=subprocess.Popen(['node','--max-semi-space-size=8',entry,'--internal-worker-prewarm'],cwd=ROOT,env=env,stdout=subprocess.PIPE,stderr=subprocess.PIPE,start_new_session=True,preexec_fn=lambda:os.sched_setaffinity(0,cpus))
